...
int client_socket, read_status;


void process_client_request(int)
{
	ClientsMap.insert( pair<int, string>(client_socket, "connected") );
	///pthread_create(&ServerThreads[serverThreadsNum-1], NULL, process_client_request, NULL);
	///serverThreadsNum++;
	
	char* aux = (char*)malloc(sizeof(char*)*500);
	printf("\nAwaiting instructions from client...\n");
	read_status = read(client_socket, &buffer, sizeof(char)*BUFF_SIZE);
	
	if (read_status < 0) { server_mutex.unlock(); return; }  // break
	if (read_status == -1) { perror("Socket read error in server"); server_mutex.unlock(); return; }  // return 1
	if (read_status == 0) { perror("Connection closed"); close(client_socket); server_mutex.unlock(); return; }  // retorn 1
	...
	{   // decode ~ { list, null, null }
		...
		{
			printf("[!] Error, buffer overflow: server_answer is too large: \"%s\" (lenght: %d)\n", server_answer, (int)strlen(server_answer));
			server_mutex.unlock(); //pthread_mutex_unlock(&server_mutex);
			return;  // return 1
		}
	}
...
int main(int argc, char** argv)
{
	...
	int running_server_PID;  // único PID activo
	
	while (true)  // siempre está esperando la conexión de algún cliente
	{
		printf("Awaiting connection...\n");
		client_socket = accept(my_socket, (struct sockaddr*)&addr, (socklen_t*)&addr);
		if (client_socket < 0)
		{
		...
		/*-- ciclo para procesar instrucciones mientras el cliente esté conectado --*/
		running_server_PID = fork();
		if (running_server_PID < 0) { perror("fork error in server"); return 1; }
		if (running_server_PID == 0)
		{
			while (true) 
			{
				...
				if (read_status < 0) { server_mutex.unlock(); break; }  // break
				if (read_status == -1) { perror("Socket read error in server"); server_mutex.unlock(); return 1; }  // return 1
				if (read_status == 0) { perror("Connection closed"); close(client_socket); server_mutex.unlock(); return 1; }  // retorn 1
				...
				//if ( *ServerThreads[serverThreadsNum-2] == 0 ) { break; }
				//else if ( *ServerThreads[serverThreadsNum-2] == 1 ) { return 1; }
